...
from civitai_search import search_civitai_loras, download_lora_from_result, format_results_html, format_detail_html, PAGE_SIZE, SORT_OPTIONS
                    ...
                    with gr.Accordion("🔍 Search Civitai LoRAs", open=False, visible=True):
                        ...
                        with gr.Row():
                            civitai_sort = gr.Dropdown(
                                choices=SORT_OPTIONS, value="Most Downloaded",
                                label="Sort by", scale=2,
                            )
                            civitai_search_btn = gr.Button("🔍 Search", variant="primary", scale=1)
                        civitai_search_status = gr.HTML()
                        civitai_results_html     = gr.HTML()
                        civitai_results_state    = gr.State([])
                        civitai_page_state       = gr.State(1)
                        civitai_totalpages_state = gr.State(1)
                        civitai_query_state      = gr.State("")
                        civitai_by_state         = gr.State("Name")
                        civitai_sort_state       = gr.State("Most Downloaded")

                        with gr.Row():
                            civitai_prev_btn = gr.Button("◀ Prev", size="sm", scale=1)
                            with gr.Column(scale=2):
                                civitai_page_info = gr.HTML("<div style='text-align:center;padding:6px;color:#888;'>Page 1</div>")
                        ...
                        def do_civitai_search(query, search_by, sort, api_key):
                            if not query.strip():
                                return ("<p style='color:orange'>⚠️ Enter a search term.</p>",
                                        [], 1, 1, query, search_by, sort,
                                        "<div style='text-align:center;padding:6px;color:#888;'>Page 1</div>",
                                        "", "")
                            results, msg, total_pages = search_civitai_loras(
                                query, search_by, api_key, page=1, page_size=PAGE_SIZE, sort=sort
                            )
                            cards  = format_results_html(results, current_page=1, total_pages=total_pages)
                            detail = format_detail_html(results[0]) if results else ""
                            page_html = f"<div style='text-align:center;padding:6px;color:#aaa;font-size:0.9em;'>Page 1 / {total_pages}</div>"
                            return cards, results, 1, total_pages, query, search_by, sort, page_html, msg, detail

                        def do_civitai_page(direction, current_page, total_pages, query, search_by, sort, api_key):
                            new_page = max(1, min(total_pages, current_page + direction))
                            results, msg, tp = search_civitai_loras(
                                query, search_by, api_key, page=new_page, page_size=PAGE_SIZE, sort=sort
                            )
                            cards  = format_results_html(results, current_page=new_page, total_pages=tp)
                            detail = format_detail_html(results[0]) if results else ""
                            page_html = f"<div style='text-align:center;padding:6px;color:#aaa;font-size:0.9em;'>Page {new_page} / {tp}</div>"
                            return cards, results, new_page, tp, page_html, msg, detail
                        ...
                        civitai_search_btn.click(
                            ...
                            inputs=[civitai_search_query, civitai_search_by, civitai_sort, civitai_api_key_gui],
                            outputs=[civitai_results_html, civitai_results_state,
                                     civitai_page_state, civitai_totalpages_state,
                                     civitai_query_state, civitai_by_state, civitai_sort_state,
                                     civitai_page_info, civitai_search_status, civitai_detail_html],
                        )
                        civitai_prev_btn.click(
                            fn=lambda pg, tp, q, by, s, key: do_civitai_page(-1, pg, tp, q, by, s, key),
                            inputs=[civitai_page_state, civitai_totalpages_state,
                                    civitai_query_state, civitai_by_state, civitai_sort_state, civitai_api_key_gui],
                            outputs=[civitai_results_html, civitai_results_state,
                                     civitai_page_state, civitai_totalpages_state,
                                     civitai_page_info, civitai_search_status, civitai_detail_html],
                        )
