...
from civitai_search import search_civitai_loras, download_lora_from_result, format_results_html, format_detail_html, PAGE_SIZE
...
with gr.Blocks(theme=args.theme, css=CSS, fill_width=True, fill_height=False) as app:
            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("LoRA", open=False, visible=True):
                    ...
                    with gr.Accordion("🔍 Search Civitai LoRAs", open=False, visible=True):
                        gr.Markdown("Search Civitai LoRAs by name or creator — NSFW included, image grid, trigger words.")
                        with gr.Row():
                            civitai_search_query = gr.Textbox(
                                label="Search",
                                placeholder="e.g. 'pants', 'micro bikini', 'stockings'...",
                                scale=3,
                            )
                            civitai_search_by = gr.Radio(
                                ["Name", "Creator"], value="Name", label="Search by", scale=1
                            )
                        civitai_search_btn    = gr.Button("🔍 Search", variant="primary")
                        civitai_search_status = gr.HTML()
                        civitai_results_html  = gr.HTML()

                        # State: results, cursor stack (list of cursors for prev),
                        # next cursor, current page number, query, search_by
                        civitai_results_state  = gr.State([])
                        civitai_cursor_stack   = gr.State([])   # stack of past cursors for Prev
                        civitai_next_cursor    = gr.State(None) # cursor for Next page
                        civitai_page_num       = gr.State(1)
                        civitai_query_state    = gr.State("")
                        civitai_by_state       = gr.State("Name")

                        with gr.Row():
                            civitai_prev_btn = gr.Button("◀ Prev", size="sm", scale=1)
                            with gr.Column(scale=2):
                                civitai_page_info = gr.HTML(
                                    "<div style='text-align:center;padding:6px;color:#888;'>—</div>"
                                )
                            civitai_next_btn = gr.Button("Next ▶", size="sm", scale=1)

                        civitai_result_index = gr.Number(
                            label="Type a result # to see details & trigger words",
                            value=1, minimum=1, precision=0,
                        )
                        civitai_detail_html = gr.HTML()
                        with gr.Row():
                            civitai_dl_btn = gr.Button("⬇️ Download Selected LoRA", variant="secondary")
                        civitai_dl_status = gr.Textbox(label="Download Status", interactive=False)

                        # ── helpers ───────────────────────────────────────────
                        def _page_html(page_num, has_next):
                            next_note = " · more available →" if has_next else ""
                            return f"<div style='text-align:center;padding:6px;color:#aaa;font-size:0.9em;'>Page {page_num}{next_note}</div>"

                        def do_civitai_search(query, search_by, api_key):
                            if not query.strip():
                                return ("<p style='color:orange'>⚠️ Enter a search term.</p>",
                                        [], [], None, 1, "", "Name",
                                        "<div style='text-align:center;padding:6px;color:#888;'>—</div>",
                                        "", "")
                            results, msg, next_cur = search_civitai_loras(
                                query, search_by, api_key, cursor=None, page_size=PAGE_SIZE
                            )
                            cards  = format_results_html(results, page_num=1)
                            detail = format_detail_html(results[0]) if results else ""
                            ph     = _page_html(1, next_cur is not None)
                            # cursor_stack starts empty (no previous pages)
                            return cards, results, [], next_cur, 1, query, search_by, ph, msg, detail

                        def do_civitai_next(results, cursor_stack, next_cursor, page_num, query, search_by, api_key):
                            if not query or next_cursor is None:
                                return (gr.update(), gr.update(), gr.update(), gr.update(),
                                        gr.update(), "⚠️ No more pages.", gr.update())
                            # Push current "start" cursor onto stack so we can go back
                            # We track the cursor that produced the current page
                            new_stack = list(cursor_stack) + [next_cursor]
                            results, msg, new_next = search_civitai_loras(
                                query, search_by, api_key, cursor=next_cursor, page_size=PAGE_SIZE
                            )
                            new_page = page_num + 1
                            cards    = format_results_html(results, page_num=new_page)
                            detail   = format_detail_html(results[0]) if results else ""
                            ph       = _page_html(new_page, new_next is not None)
                            return cards, results, new_stack, new_next, new_page, ph, msg, detail

                        def do_civitai_prev(results, cursor_stack, page_num, query, search_by, api_key):
                            if not query or not cursor_stack:
                                return (gr.update(), gr.update(), gr.update(), gr.update(),
                                        gr.update(), "⚠️ Already on first page.", gr.update())
                            # Pop stack to get the cursor for the previous page
                            new_stack   = list(cursor_stack)
                            prev_cursor = new_stack.pop() if len(new_stack) > 1 else None
                            use_cursor  = new_stack[-1] if new_stack else None
                            # For page 2→1 use no cursor (fresh search)
                            if page_num <= 2:
                                use_cursor = None
                                new_stack  = []
                            results, msg, new_next = search_civitai_loras(
                                query, search_by, api_key, cursor=use_cursor, page_size=PAGE_SIZE
                            )
                        ...
                        def do_show_detail(results, index):
                            if not results:
                                return ""
                            idx = int(index) - 1
                            if idx < 0 or idx >= len(results):
                                return f"<p style='color:orange'>Pick 1 to {len(results)}.</p>"
                            return format_detail_html(results[idx])

                        def do_civitai_download(results, index, api_key):
                            if not results:
                                return "❌ Search first.", *[gr.update()] * 7
                            idx = int(index) - 1
                            if idx < 0 or idx >= len(results):
                                return f"❌ Pick 1 to {len(results)}.", *[gr.update()] * 7
                            ...
                            path, msg = download_lora_from_result(result, DIRECTORY_LORAS, api_key)
                            new_list  = get_model_list(DIRECTORY_LORAS)
                            new_list.insert(0, "None")
                            new_list  = new_list + DIFFUSERS_FORMAT_LORAS
                            dd = gr.update(choices=new_list)
                            return msg, dd, dd, dd, dd, dd, dd, dd

                        # ── wiring ────────────────────────────────────────────
                        _search_outputs = [
                            civitai_results_html, civitai_results_state,
                            civitai_cursor_stack, civitai_next_cursor,
                            civitai_page_num, civitai_query_state, civitai_by_state,
                            civitai_page_info, civitai_search_status, civitai_detail_html,
                        ]
                        _nav_outputs = [
                            civitai_results_html, civitai_results_state,
                            civitai_cursor_stack, civitai_next_cursor,
                            civitai_page_num, civitai_page_info,
                            civitai_search_status, civitai_detail_html,
                        ]
                        ...
                        civitai_next_btn.click(
                            fn=do_civitai_next,
                            inputs=[civitai_results_state, civitai_cursor_stack,
                                    civitai_next_cursor, civitai_page_num,
                                    civitai_query_state, civitai_by_state, civitai_api_key_gui],
                        ...
                        civitai_result_index.change(
                            fn=do_show_detail,
                            inputs=[civitai_results_state, civitai_result_index],
                            outputs=[civitai_detail_html],
                        )
                        civitai_dl_btn.click(
                            ...
                            outputs=[civitai_dl_status,
                                     lora1_gui, lora2_gui, lora3_gui, lora4_gui,
                                     lora5_gui, lora6_gui, lora7_gui],
                        )
    ...
    with gr.Tab("🗂️ Image History"):
        gr.Markdown("### 🗂️ Image History — All generated images this session")
        gr.Markdown("Click **Refresh** to load the latest images. Click any image to preview and download.")
        with gr.Row():
        ...
        def load_image_history():
            ...
            folders = [
                "./images",
                "./up_images",
                "/content/drive/MyDrive/DiffuseCraft_Outputs1",
            ]
            ...
            if not images:
                return [], "<p>No images found yet. Generate some images first!</p>"
            return images, f"<p>Found <b>{len(images)}</b> image(s).</p>"
        refresh_btn.click(
            fn=load_image_history,
            inputs=[],
