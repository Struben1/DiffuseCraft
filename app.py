            ...
            with gr.Column(scale=1):
                ...
                with gr.Accordion("LoRA", open=False, visible=True):
                    ...
                    with gr.Accordion("🔍 Search Civitai LoRAs", open=False, visible=True):
                        gr.Markdown("Search Civitai by name or creator — NSFW included. 3 cards per row with preview images.")
                        ...
                        # Results grid (has internal scrollbar — no page scrolling needed)
                        ...
                        # Pagination controls right below the grid
                        with gr.Row():
                            civitai_prev_btn  = gr.Button("◀ Prev", size="sm", scale=1)
                            civitai_page_info = gr.HTML("<div style='text-align:center;padding:6px;color:#888;'>Page 1</div>", scale=2)
                        ...
                        def do_civitai_search(query, search_by, api_key):
                            ...
                            results, msg, total_pages = search_civitai_loras(
                                query, search_by, api_key, page=1, page_size=PAGE_SIZE
                            )
                            ...
                            return cards, results, 1, total_pages, query, search_by, page_html, msg, detail

                        def do_civitai_page(direction, current_page, total_pages, query, search_by, api_key):
                            new_page = max(1, min(total_pages, current_page + direction))
                            results, msg, tp = search_civitai_loras(
                                query, search_by, api_key, page=new_page, page_size=PAGE_SIZE
                            )
                        ...
                        civitai_search_btn.click(
                            ...
                            outputs=[
                                ...
                                civitai_query_state, civitai_by_state,
                                civitai_page_info, civitai_search_status, civitai_detail_html,
                            ],
                        )
                        civitai_prev_btn.click(
                            fn=lambda pg, tp, q, by, key: do_civitai_page(-1, pg, tp, q, by, key),
